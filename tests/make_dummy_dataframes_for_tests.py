...
SHUFFLE = [[1, 3], [1, 4], [3, 4], [0, 1, 3], [0, 1, 4], [0, 3, 4],[1, 3, 4], [0, 1, 3, 4]]
"""
...
def classify_df_as_0(df, df_pt):
    """Retuns df and df_pt_cut reclassified as 0"""
...
def replace_values_in_points_and_grids(df, df_pt, col_name, array_to_replace, new_classification):
    """Sets input df_pt to length of array_to_replace and replaces p values using array_to_replace"""
    df = df.reset_index(drop=True)
    if new_classification == 0:
        df, df_pt_cut = classify_df_as_0(df, df_pt)
    else:
        df, df_pt_cut, df_pt_to_replace = reclassify_using_array(df,
                                                                 df_pt,
                                                                 array_to_replace,
                                                                 new_classification)
    return df, df_pt_cut
...
def make_global_df(df_final, df_pt_final, df_GLOBAL):
    ...
    if not df_pt_final.empty:
        df_pt_agg = gridgran.aggregrid(df_pt_final, CLASSIFICATION_DICT)[['ID125m', 'p', 'h',
                                                 'p_cls', 'h_cls',
        ...
        df_pt_agg = df_pt_final.copy()[['ID125m', 'p', 'h']].set_index('ID125m')
        df_grid_GLOBAL = df_GLOBAL[
            ['ID125m', 'ID250m', 'ID500m', 'ID1000m',
             'dissolve_id']].set_index(
            'ID125m').join(df_pt_agg)
    ...
    df_grid_GLOBAL['dissolve_id'] = np.nan #Move dissolve_id to end of columns
...
def make_any_combination(df, df_pt, df_GLOBAL,  cell_config, upper_lower='upper',
                         remake_cls_4=False):
    """Returns dataframe with cells classified to combination in cell_fig, along with corresponding points matching class criteria"""
