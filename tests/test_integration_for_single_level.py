...
def test_int_single_level(dfs, cell_configs):
    ...
    if np.any(cell_configs == 2) or np.all(cell_configs == 1) or np.all(
            cell_configs == 3) or sorted(list(np.unique(cell_configs))) == [0, 1] or sorted(list(np.unique(
            cell_configs))) == [0, 3]: #Aggregate up
